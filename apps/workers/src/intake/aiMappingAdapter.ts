...
function buildDecisionLog(
  ...
  batch?: MappingSuggestionBatch,
  overrides: Partial<Omit<MappingDecisionLog, "provider" | "model" | "promptVersion" | "validationOutcome" | "confidence">> = {},
): MappingDecisionLog {
  return {
    provider,
    model,
...
function hasLowConfidence(batch: MappingSuggestionBatch): boolean {
  return (
    ...
    batch.suggestions.some(
      (suggestion) => suggestion.confidence < LOW_CONFIDENCE_THRESHOLD,
    )
  );
}
    ...
    batch.suggestions.map(async (suggestion) => {
      try {
        const rawAssessment = await runner<JudgeAssessment>({
          ...
          schemaDescription:
            "Advisory human-review recommendation for one mapping suggestion.",
...
export async function suggestMappings(
  ...
  const primaryMaxAttempts =
    dependencies.primaryMaxAttempts ?? DEFAULT_PRIMARY_MAX_ATTEMPTS;
  const retryDelayMs = dependencies.retryDelayMs ?? DEFAULT_RETRY_DELAY_MS;

  let primaryRunner: StructuredRunner;
  try {
    primaryRunner =
      dependencies.primaryRunner ??
      createWorkersStructuredRunner(dependencies.env ?? {});
  } catch (error) {
    const reason =
      error instanceof Error ? error.message : "Workers AI binding is unavailable";
    return {
      kind: "abstain",
      reason,
  ...
  try {
    ...
    return {
      kind: "runtime_failure",
      reason:
        error instanceof Error ? error.message : "Primary model execution failed.",
      decisionLog: buildDecisionLog(
        provider,
        primaryModel,
        input.promptVersion,

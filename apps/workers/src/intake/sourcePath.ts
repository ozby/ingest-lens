...
export function parseSourcePath(path: string): SourcePathError | { ok: true; segments: string[] } {
  if (!path.startsWith("/")) {
    return createError("invalid_syntax", path, "Source paths must start with '/'.");
  }
  ...
  if (rawSegments.some((segment) => segment.length === 0)) {
    return createError("invalid_syntax", path, "Source paths cannot contain empty path segments.");
  }
  ...
  for (const segment of segments) {
    ...
    if (segment === "." || segment === ".." || segment.includes("*") || segment === "-") {
      return createError(
        "unsupported_traversal",
        path,

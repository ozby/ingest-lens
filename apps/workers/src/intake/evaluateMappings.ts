...
function scoreExactMapping(
  task: MappingEvalTask,
...
function scoreNonHallucination(
  task: MappingEvalTask,
  batch: MappingSuggestionBatch,
): boolean {
  return batch.suggestions.every((suggestion) =>
    resolveSourcePath(task.source_payload, normalizeTaskPath(suggestion.sourcePath)).ok,
  );
}

function scoreCorrectAbstention(
  task: MappingEvalTask,
  batch: MappingSuggestionBatch,
): number {
...
export function createGoldenEvalBatch(
  task: MappingEvalTask,
  ...
  return {
    ...
    suggestions: Object.entries(task.expected_mapping).map(
      ([targetField, sourcePath], index) => ({
...
export function evaluateMappings(
  ...
  const nonHallucinationRate =
    taskResults.length === 0
      ? 1
      : taskResults.filter((result) => result.nonHallucinationPass).length /
        taskResults.length;
  ...
  return {
    ...
    pass:
      evalWeightedScore >= 0.75 &&
      adversarialWeightedScore >= 0.6 &&
      nonHallucinationRate >= 0.75,
    taskResults,
  };
}

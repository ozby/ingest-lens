...
export interface ValidateIntakeRequestDependencies
  extends DeterministicDependencies {
...
export function validateIntakeRequest(
  ...
  const hasFixtureId =
    typeof input.fixtureId === "string" && input.fixtureId.trim().length > 0;
    ...
    sourceFixture === undefined
      ? new Date(
          dependencies.clock().getTime() +
            (dependencies.reviewPayloadTtlHours ??
              DEFAULT_REVIEW_PAYLOAD_TTL_HOURS) *
              60 *
              60 *
              1000,

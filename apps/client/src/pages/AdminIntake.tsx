...
const AdminIntake = () => {
  ...
  const handleApprove = async (attempt: IntakeAttemptRecord) => {
    ...
    try {
      ...
      setAttempts((current) =>
        current.map((item) =>
          item.intakeAttemptId === approval.attempt.intakeAttemptId
            ? approval.attempt
            : item,
        ),
      );
  ...
  const handleReject = async (event: FormEvent<HTMLFormElement>, attempt: IntakeAttemptRecord) => {
    ...
    try {
      ...
      setAttempts((current) =>
        current.map((item) =>
          item.intakeAttemptId === updated.intakeAttemptId ? updated : item,
        ),
      );
      ...
      <div className="space-y-3">
        {attempt.suggestionBatch.suggestions.map((suggestion: MappingSuggestion) => (
          <MappingSuggestionReview
            ...
            checked={(selectedSuggestions[attempt.intakeAttemptId] ?? []).includes(
              suggestion.id,
            )}
                  ...
                  <CardHeader>
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="text-xs">
                        <Badge className={statusTone[attempt.status]}>
                          {attempt.status}
                        </Badge>

...
The API Worker (`node-pubsub-dev`) and the client Worker (`node-pubsub-client-dev`) are separate CF Workers. Each can be rolled back independently.
...
```
Successfully rolled back to the previous version of node-pubsub-client-dev
```
...
Symptom: `https://dev.ozby.dev` loads but authenticated API calls fail with a CORS error in the browser console.

Diagnosis:

```bash
curl -s -I -H "Origin: https://dev.ozby.dev" https://api.dev.ozby.dev/health | grep -i "access-control"
```
...
1. Confirm `[env.dev.vars] ALLOWED_ORIGIN = "https://dev.ozby.dev"` in `apps/workers/wrangler.toml`
...
```bash
curl -s -I -H "Origin: https://dev.ozby.dev" https://api.dev.ozby.dev/health | grep -i "access-control"
# expected: Access-Control-Allow-Origin: https://dev.ozby.dev
```

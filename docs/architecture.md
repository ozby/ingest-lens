...
with KV jti revocation; rate-limited via `API` and `AUTH_RATE_LIMITER`
bindings.
...
3. 2xx → `ack`; 5xx/network → `retry({ delaySeconds })` with backoff.
4. Missing row → `ack` (safe drop).

`max_retries = 5` then `delivery-dlq`. **At-least-once.** Receivers must
be idempotent; publishers can dedupe via `Idempotency-Key`.
...
sequenceDiagram
  ...
  W->>PG: INSERT message (idempotency check)
...
POST /api/messages/:queueId
  → auth → idempotency check → INSERT message
...
POST /api/topics/:topicId/publish
  ...
  → for each queue: INSERT message; if pushEndpoint: DELIVERY_QUEUE.send({ ..., topicId })
```
...
DELIVERY_QUEUE batch (≤10)
  ...
  → POST pushEndpoint
  → 2xx: ack; if topicId: notify TopicRoom { messageId, seq, queueId, topicId }
...
flowchart LR
  ...
  L --> KS{KillSwitchKV}
  KS -->|off| X[404]
  KS -->|on| SL[SessionLock DO]
  SL --> G[LabConcurrencyGauge DO]
  ...
  HC[HeartbeatCron 15m / 10k weekly] --> S1A
  CE[CostEstimatorCron $50/day] -->|auto-flip| KS
```

```text
Browser → apps/lab (Hono SSR + htmx)
  ├ KillSwitchKV middleware (404 the surface when off)
  ├ SessionLock DO (alarm-backed TTL, single-writer per scenario)
  ├ LabConcurrencyGauge DO (global cap: 100 sessions)
  ...
  ├ HeartbeatCron (15-min synthetic; 10k weekly)
  └ Workers Assets — CSS + htmx.min.js
```
...
| `apps/lab`            | Hono SSR shell, Workers Assets, crons, S1a/S1b runner DOs                |

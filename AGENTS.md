...
<!-- AGENT SURFACE — DO NOT REMOVE -->

## Agent surface

`.agent/` is the repo-owned source of truth for commands, workflows, and skills.
Derived surfaces (`.claude/`, `.cursor/`, `.windsurf/`, `.opencode/`,
`.agents/skills/`, and `.gemini/`) exist only to expose that same content in
consumer-native layouts.

Rules:

- **Edit `.agent/` first.** Treat it as canonical.
- **Do not casually hand-edit derived surfaces.** Update them only when this
  task explicitly includes a live convention change that must land in the checked-in mirror.
- **Prefer repo-owned conventions over external tool branding.** Describe the
  repo's command, workflow, and skill surfaces directly instead of delegating to
  upstream scaffolding docs.

Invocation by tool:

- Claude / OpenCode / Cursor / Windsurf: `/name` slash command.
- Codex (native): `$name` — skills live under `.agents/skills/<name>/SKILL.md`.
- Gemini CLI: `/name` — mirrored command surface under `.gemini/commands/`.

...
- **`.agent/`** (repo-owned) — the canonical surface. Every slash command,
  workflow, and skill lives here as markdown. Edit here when local needs
  diverge from the catalog. Commit the changes.
- **Derived surfaces** (`.claude/`, `.cursor/`, `.windsurf/`, `.opencode/`,
  `.agents/skills/`, `.gemini/`) — regenerated by `npx ak symlink sync`.
  Never edit by hand. Each mirror uses its tool's native convention:
  - **Commands**: per-file `.md` symlinks under `.{tool}/commands/*.md`
    for Claude / Cursor / Windsurf / OpenCode.
  - **Claude skills**: one directory symlink (`.claude/skills →
.agent/skills`). OpenCode also reads this as a fallback.
  - **Cross-tool skills**: per-skill symlinks under `.agents/skills/<name>`.
    Serves Codex CLI (official project path per
    [OpenAI's "Where to save skills"](https://developers.openai.com/codex/skills)),
    Amp (Sourcegraph, official project path), and OpenCode via its
    `.agents/skills/` fallback — one surface, three tools.
  - **Gemini**: transformed TOML copies at `.gemini/commands/*.toml`.
  - **Not mapped**: `.codex/prompts/` (deprecated by OpenAI + home-only
    even when it existed), `.codex/skills/` (Codex doesn't read there —
    despite what some third-party tools claim), `.opencode/skills/`
    (redundant with the `.claude/skills/` and `.agents/skills/` fallbacks
    OpenCode already reads).
- **Catalog** (read-only, in `node_modules/@webpresso/agent-kit/catalog/`)
  — the template. Bump the package + `npx ak init --overwrite` to pull new
  content, then re-run `npx ak symlink sync`.

Invocation by tool:

- Claude / OpenCode / Cursor / Windsurf: `/name` slash command.
- Codex (native): `$name` — implicitly or explicitly invokes skills from

...
Validate the refined artifact directly from the repo-owned blueprint file. If the refinement changes markdown structure enough to warrant a lint pass, run the repo's markdown linter against `blueprints/*/<slug>/_overview.md` and, if present, the repo's blueprint parser/validator tests.

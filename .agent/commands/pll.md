...
Execute Blueprint-shaped work through the current parallel engine layer while keeping durable lifecycle state on repo-owned blueprint files and status metadata.

`/pll` is an **operator adapter**, not a standalone runtime. It should agree with the repo's durable blueprint files and lifecycle conventions instead of inventing a separate execution model.

It does three things:

1. understands task dependencies well enough to preserve ordering
2. keeps Blueprint lifecycle state honest via repo-owned status updates in the blueprint artifact
...
- mark the blueprint as started before execution begins
- mark each task as in progress, blocked, or complete in the durable blueprint state surface
- record block reasons next to the affected task, not in ad hoc chat state
- finalize the blueprint only after every acceptance and verification gate actually passes
- treat structural moves as recovery-only, not the normal execution primitive
...
`/pll` and any non-interactive blueprint executor should remain two surfaces over one repo-owned model:
...
- Use the blueprint markdown itself plus its named verification checks to prove state transitions.

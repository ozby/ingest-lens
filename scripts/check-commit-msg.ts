...
if (!SUBJECT_RE.test(subject)) fail(`subject does not match conventional-commit format: "${subject}"`);
...
if (subject.includes("[lore]")) {
  ...
  if (!/^(Constraint|Rejected|Directive):\s+\S+/m.test(body)) {
    fail("`[lore]` commits require at least one of `Constraint:`, `Rejected:`, or `Directive:` trailers");
  }
}

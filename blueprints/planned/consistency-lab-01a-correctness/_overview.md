...
workload runs simultaneously through three delivery paths (Cloudflare Queues,
Postgres polling with `ORDER BY`, Hyperdrive-backed `LISTEN/NOTIFY`), and the
lab reports per-path **delivered count, duplicate count, inversion count,
ordering property, and status**. This is Lane B. Produces the first
screenshotable "whoa" result of the lab.
...
- **Scope:** Three `ScenarioRunner` implementations (`CfQueuesPath`,
  `PgPollingPath`, `HyperdriveNotifyPath`), a parent runner hosted in a
...
POST /lab/s1a/run  (sessionId, cookie issued by shell Lane D)
 ...
 S1aRunnerDO.start(sessionId, workloadSize=1000, seed=<rng-seed>, mode="sequential")
       ...
       │
       ▼
 for each path in [CfQueues, PgPolling, HyperdriveNotify]:
   ...
   │                                                    ▼ record recv_order
   │
   └─ HyperdriveNotifyPath ── INSERT + NOTIFY batch ──▶ subscriber (DO holding the pg conn, F1T)
                                                                │
                                                                ▼ record recv_order
       │
       ▼
...
| Third path: direct PG connection (NOT Hyperdrive) | Subscriber DO opens `connect()`-based TCP to Postgres; Hyperdrive docs explicitly state LISTEN/NOTIFY is unsupported | Makes the Hyperdrive limitation part of the scenario's finding, not a hidden assumption                                                                                                                                                                                             | F1T-reversed  |

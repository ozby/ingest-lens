...
- [`client-route-code-splitting`](./planned/client-route-code-splitting/_overview.md) — split the client SPA at route boundaries for faster first paint (warning-elimination premise reframed after the Workers Assets decision; see blueprint preface).
- [`client-workers-assets-deploy`](./planned/client-workers-assets-deploy/_overview.md) — deploy the client SPA as a pure-static Worker via wrangler's `[assets]` binding at `dev.ozby.dev` (prd: `ozby.dev`); CF-official Workers + Assets pattern, atomic custom domain + DNS + cert, CORS to the API Worker.
